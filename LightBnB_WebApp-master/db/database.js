...
const getAllReservations = function(guest_id, limit = 10) {
  const queryName = `SELECT reservations.*, properties.*, avg(rating) as average_rating
  FROM reservations 
  ...
  return pool
    .query(queryName, [guest_id, limit])
    .then((result) => {
      return result.rows;
    })
    .catch((err) => {
...
const getAllProperties = function(options, limit = 10) {
  ...
  return pool.query(queryString, queryParams)
    .then((res) => {
      ...
      console.log('typeof average_rating: ', typeof res.rows[0].average_rating);
      return res.rows;
    });
};
